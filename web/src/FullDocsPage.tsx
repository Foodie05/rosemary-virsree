...
const samples={
  curl:`# 第 1 步：只向 Rosemary 发送 JSON，申请短期上传地址
...
# 第 2 步：把文件 PUT 到响应中的短期 url；根据 direct 判断 S3 直达或 WebDAV 中转
...
// signed.direct=true 时直达 S3/CDN；false 时由 WebDAV 能力地址中转；URL 不要写日志
...
# 文件正文只发送到 signed.url；检查 signed.direct
...
<section id="upload"><div className="section-icon"><Upload/></div><h3>上传：申请地址 → PUT → commit</h3><p>上传永远是三步。第一步和第三步只传 JSON 到 Rosemary；第二步才传文件，目标必须是响应 URL。S3 来源直达真实存储，WebDAV 来源会明确标记为中转。</p><div className="mini-flow"><span>① 申请签名<small>JSON → Rosemary</small></span><ArrowRight/><span>② PUT 文件<small>bytes → signed.url</small></span><ArrowRight/><span>③ commit<small>JSON → Rosemary</small></span></div><div className="lang-tabs">{Object.keys(samples).map(k=><button className={lang===k?'active':''} key={k} onClick={()=>setLang(k as keyof typeof samples)}>{k==='curl'?'HTTP / cURL':k==='ts'?'TypeScript':'Python'}</button>)}</div><Code>{samples[lang]}</Code><div className="warning-box"><AlertTriangle/><div><strong>不要调用 <code>PutObject({gateway}/s3/...)</code></strong><p>它会返回 405。只有第二步允许 PUT，且目标必须是签名响应中的 URL。</p></div></div></section>
<section id="download"><div className="section-icon"><Link2/></div><h3>下载与公开链接</h3><p>调用 <code>POST /api/v1/buckets/{'{bucket}'}/objects/download</code>，提交 <code>key</code> 和本次业务选择的 <code>expires_in</code>。响应包含 direct：true 时 URL 指向真实私有 S3/CDN；false 时对象来自 WebDAV，URL 经 Rosemary 中转。</p><Code>{`POST ${gateway}/api/v1/buckets/media/objects/download\nX-RVS-Access-Key: <virtual AK>\nX-RVS-Secret-Key: <virtual SK>\nContent-Type: application/json\n\n{"key":"reports/q3.pdf","filename":"q3.pdf","expires_in":300}`}</Code><p>“公开链接”仍然基于签名：应用创建稳定的 <code>/p/...</code> 地址；访问时 Rosemary 重新签发并 307 跳转。保存创建响应的 <code>slug</code>，用 <code>DELETE .../public-links/{'{slug}'}</code> 撤销。底层桶始终 Private。</p></section>
...
<section id="verify"><div className="section-icon"><Check/></div><h3>交付前逐项验收</h3><ol className="checklist"><li>健康检查显示真实后端 Ready。</li><li>凭据文件权限为 0600，日志和 Git diff 中没有 Token、AK/SK。</li><li>ListObjectsV2 与 HeadObject 正常。</li><li>直接向 Rosemary PutObject 返回 405。</li><li>检查上传响应 direct；true 时主机不是 Rosemary，false 时确认是预期 WebDAV 中转。</li><li>对响应 URL PUT 成功，commit 成功。</li><li>下载内容一致，并按 direct 验证 S3/CDN 直达或 WebDAV 中转。</li><li>没有 write 权限的 Key 申请上传返回 403。</li></ol></section>
<section id="errors"><div className="section-icon"><Terminal/></div><h3>遇到错误先看这里</h3><table className="error-table"><tbody>{[['backend_ready=false','尚未添加可用存储源，请联系平台管理员'],['403 invalid credentials','虚拟凭据错误、解密主密钥变化或 Key 已撤销'],['403 permission denied','当前 Key 没有所需的独立权限'],['expires_in is required','应用必须为本次签名明确选择秒数'],['bucket quota exceeded','清理对象、等待过期预留回收或调整配额'],['commit 大小不匹配','申请时 size 与真实上传字节数不同'],['浏览器 CORS 失败','真实 S3 桶需要允许应用域名的 GET/HEAD/PUT'],['PutObject 返回 405','这是预期行为，请改用签名上传三步流程']].map(([e,d])=><tr key={e}><td><code>{e}</code></td><td>{d}</td></tr>)}</tbody></table><p>完整接口字段见 <a href={`${gateway}/docs/api.md`} target="_blank">API 文档</a>；仍无法解决时，在 <a href={`${project}/issues`} target="_blank">GitHub Issues</a> 提交脱敏后的复现。</p></section>
</main></div></div>}
