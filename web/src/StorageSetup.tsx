...
import {API, StorageSource, StorageSourceDetail, StorageSourceInput} from './api';
...
function StorageSourceForm({submitLabel,onSaved,initial}:{submitLabel:string;onSaved:()=>void;initial?:StorageSourceDetail}){
  ...
  const save=async(acknowledgeBucketChange=false)=>{
    ...
    if(kind==='s3')Object.assign(payload,{public_endpoint:cleanPublicEndpoint,region,bucket:bucket.trim(),access_key:accessKey,secret_key:secretKey,path_style:pathStyle,cdn_endpoint:cleanCDN,acknowledge_bucket_change:acknowledgeBucketChange});
