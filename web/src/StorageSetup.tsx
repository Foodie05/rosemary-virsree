...
export function StoragePage({notify}:{notify:(s:string)=>void}){
  ...
  const load=()=>{setLoading(true);setError('');Promise.all([api.storageSources(),api.platformFilesystem()]).then(([v,fs])=>{setSources(v||[]);setFilesystem(fs)}).catch(e=>setError(e.message)).finally(()=>setLoading(false))};
  useEffect(load,[]);
