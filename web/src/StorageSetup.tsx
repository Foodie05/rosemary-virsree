...
import {API} from './api';
import type {CDNMode, PlatformFilesystem, StorageSource, StorageSourceDetail, StorageSourceInput} from './api';
...
function StorageModal({eyebrow,title,close,children,busy=false}:{eyebrow:string;title:string;close:()=>void;children:ReactNode;busy?:boolean}){
  useVersionHold();
  return <div className="modal-bg"><div className="storage-modal" role="dialog" aria-modal="true" aria-label={title}><div className="modal-head"><div><p className="eyebrow">{eyebrow}</p><h3>{title}</h3></div><button aria-label="关闭" disabled={busy} onClick={close}><X/></button></div>{children}</div></div>;
}
...
export function StoragePage({notify}:{notify:(s:string)=>void}){
  ...
  const[filesystem,setFilesystem]=useState<PlatformFilesystem>();
  ...
  const load=()=>{setLoading(true);setError('');Promise.allSettled([api.storageSources(),api.platformFilesystem()]).then(([sourceResult,filesystemResult])=>{const problems:string[]=[];if(sourceResult.status==='fulfilled')setSources(sourceResult.value||[]);else problems.push('存储源：'+sourceResult.reason.message);if(filesystemResult.status==='fulfilled')setFilesystem(filesystemResult.value);else problems.push('平台文件系统：'+filesystemResult.reason.message);setError(problems.join('；'))}).finally(()=>setLoading(false))};
  useEffect(load,[]);
  ...
  return <><div className="section-head"><div><p className="eyebrow">PHYSICAL STORAGE POOL</p><h2>存储源</h2><p>按优先级分配新对象；最高优先级来源同时承载 VirSree 平台文件系统。</p></div><button className="primary small" onClick={()=>setOpen(true)}><Plus size={16}/>添加存储源</button></div>{error&&<div className="error" role="alert"><ShieldCheck/>{error}<button className="ghost" onClick={load}>重试</button></div>}{!loading&&filesystem&&<article className={`platform-fs ${filesystem.initialized?'ready':''}`}><div><ShieldCheck/><span><small>VIRSREE PLATFORM FILESYSTEM</small><strong>{filesystem.initialized?'加密滚动备份正在运行':'正在建立平台文件系统'}</strong></span></div><p>元数据、审计日志和对象键映射每 {filesystem.backup_interval_seconds>=3600?`${filesystem.backup_interval_seconds/3600} 小时`:`${Math.ceil(filesystem.backup_interval_seconds/60)} 分钟`}备份至 <code>{filesystem.source_name||'最高优先级存储源'}/{filesystem.prefix}</code>，保留 {filesystem.retention} 个滚动快照。</p><span>{filesystem.last_success_at?`最近成功：${new Date(filesystem.last_success_at).toLocaleString()}`:filesystem.last_error_code?`等待重试 · ${filesystem.last_error_code}`:'等待首次备份'}</span></article>}{loading?<div className="skeletons"><i/><i/><i/><i/></div>:<div className="source-grid">{sources.map((s,index)=>{const used=s.used_bytes+s.reserved_bytes,pct=s.capacity_unlimited?0:Math.min(100,used/s.capacity_bytes*100);return <article className="source-card" key={s.id}><div className="source-rank"><span>优先级 {s.priority}</span>{index===0&&<b>平台文件系统</b>}</div><div className="source-title"><div>{s.kind==='s3'?<Cloud/>:<Globe2/>}</div><div><h3>{s.name}</h3><code>{s.kind.toUpperCase()}</code></div><span className="status"><i/>已通过接入验证</span></div><div className="source-capacity"><div><span>{gb(used)} 已占用</span><span>{s.capacity_unlimited?'∞ 无上限':gb(s.capacity_bytes)}</span></div><div className={`bar ${s.capacity_unlimited?'unlimited':''}`}><i style={{width:`${pct}%`}}/></div></div><footer><span>{s.direct_transfer?'文件直达存储':'经 VirSree 安全中转'}</span><div>{s.cdn_enabled&&<b>CDN 下载</b>}<button className="source-edit" aria-label={`编辑${s.name}`} disabled={editingID===s.id} onClick={()=>beginEdit(s.id)}>{editingID===s.id?<Loader2 className="spin"/>:<Pencil/>}编辑</button></div></footer></article>})}</div>}{open&&<StorageModal eyebrow="NEW STORAGE SOURCE" title="添加存储源" busy={formBusy} close={()=>setOpen(false)}><StorageSourceForm onBusyChange={setFormBusy} submitLabel="验证并添加" onSaved={()=>{setOpen(false);load();notify('存储源已验证并加入调度池')}}/></StorageModal>}{editing&&<StorageModal eyebrow="EDIT STORAGE SOURCE" title="编辑存储源" busy={formBusy} close={()=>setEditing(null)}><div className="edit-verification-note"><ShieldCheck/><span><strong>更新前重新验证</strong><small>新配置通过完整读、写、复制和删除验证后才会生效；验证失败时原配置继续运行。</small></span></div><StorageSourceForm initial={editing} onBusyChange={setFormBusy} submitLabel="重新验证并保存" onSaved={()=>{setEditing(null);load();notify('存储源已重新验证并更新')}}/></StorageModal>}</>;
}

function StorageSourceForm({submitLabel,onSaved,initial,onBusyChange}:{submitLabel:string;onSaved:()=>void;initial?:StorageSourceDetail;onBusyChange?:(busy:boolean)=>void}){
  ...
  const[capacityUnlimited,setCapacityUnlimited]=useState(initial?.capacity_unlimited??false);
  ...
  const save=async(acknowledgeBucketChange=false)=>{
    ...
    const payload:StorageSourceInput={name,kind,priority,capacity_bytes:capacityUnlimited?0:Math.round(capacity*2**30),capacity_unlimited:capacityUnlimited,endpoint:cleanEndpoint};
    ...
    try{
      ...
      onSaved();
    }catch(e:any){setError(e.message)}finally{setBusy(false);onBusyChange?.(false)}
  };
