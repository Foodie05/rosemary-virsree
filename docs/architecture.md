...
A real presigned URL cannot be recalled because authorization is embedded in its query string. For immediate object-level invalidation, Rosemary copies the current object to a new opaque physical key inside S3, deletes the old physical key, and updates its logical mapping. Stable aliases resolve through the mapping and continue unless revoked through their own API; previously returned direct URLs fail.
